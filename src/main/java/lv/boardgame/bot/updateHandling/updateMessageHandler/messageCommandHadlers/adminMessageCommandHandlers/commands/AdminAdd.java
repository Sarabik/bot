...
import static lv.boardgame.bot.constants.TextFinals.ADMIN_ADDED;
...
@Component
public class AdminAdd implements MessageCommand {
	...
	private AdminAdd(final AdminService adminService) {
	...
	public List<SendMessage> execute(final String chatId, final Player player, final String receivedText) {
		...
		String newAdminUsername = receivedText.split(" ")[1];
		Admin admin = adminService.saveAdmin(new Admin(newAdminUsername));
		return List.of(getCustomMessage(chatId, ADMIN_ADDED + admin.getUsername()));
	}
}
